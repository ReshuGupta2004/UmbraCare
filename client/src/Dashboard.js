...
const fetchPredictionData = async () => {
  try {
    ...
    const response = await axios.get(`http://localhost:5000/api/period-tracker/predictio/${userId}`, {
      headers: {
        'x-auth-token': token
      }
    });
                ...
                data={[
                  {
                    ...
                    mode: 'number+gauge+delta',
                    value: 5,
                    delta: { reference: 40, decreasing: { color: '#B85170' }, font: { size: 14 } }, // Changed to primary color
                    domain: { 'x': [0.05, 0.95], 'y': [0.88, 0.98] },
                    title: { 'text': `<b style="font-size:18px;color:#B85170">5 Weeks Pregnant</b><br><span style="font-size:14px;color:#666">35 Weeks to Go</span>`, 'font': { 'family': 'Poppins' } }, // Changed to primary color
                    gauge: {
                      'shape': "bullet",
                      'axis': { 'range': [null, 40], 'visible': false },
                  ...
                  },
                  {
                    ...
                    mode: 'number',
                    value: pregnancyData.sizes[4],
                    number: { 'suffix': " inches", 'font': { 'size': 28, 'color': '#B85170', 'family': 'Poppins' } }, // Changed to primary color
                    domain: { 'x': [0.05, 0.45], 'y': [0.6, 0.8] },
                    title: { 'text': "<b>Current Size</b>", 'font': { 'size': 14, 'family': 'Poppins' } }
                  },
                  {
                    ...
                    mode: 'number',
                    value: 0,
                    number: { 'prefix': `<span style="font-size:20px;color:#B85170">👶 Baby size like:<br><span style="font-size:24px;font-weight:bold">${pregnancyData.comparisons[4]}</span></span>` }, // Changed to primary color
                    domain: { 'x': [0.55, 0.95], 'y': [0.6, 0.8] }
                  },
                  {
                    type: 'scatter',
                    ...
                    y: pregnancyData.sizes,
                    mode: 'lines+markers',
                    line: { 'color': 'rgba(184, 81, 112, 0.7)', 'width': 4, 'shape': 'spline', 'smoothing': 1.2 }, // Adjusted to primary color shade
                    marker: {
                      'size': pregnancyData.sizes.map(s => Math.max(8, s * 15)),
                      'color': pregnancyData.sizes.map((s, i) => i === 4 ? '#B85170' : 'rgba(184, 81, 112, 0.5)'), // Changed to primary color and shade
                      'line': { 'width': 1, 'color': 'white' },
                      'symbol': pregnancyData.weeks.map((w, i) => i === 4 ? 'diamond' : 'circle'),
                      'opacity': 0.8
                    },
                  ...
                  },
                  {
                    ...
                    y: [pregnancyData.sizes[4]],
                    mode: 'markers',
                    marker: { 'size': 30, 'color': '#B85170', 'symbol': 'diamond', 'line': { 'width': 2, 'color': 'white' }, 'opacity': 0.9 }, // Changed to primary color
                    'hoverinfo': 'skip'
                  }
                ]}
                layout={{
                ...
                data={[
                  {
                    ...
                    x: ['Week 4', 'Week 8', 'Week 12', 'Week 16', 'Week 20', 'Week 24', 'Week 28', 'Week 32', 'Week 36', 'Week 40'],
                    y: [72, 75, 78, 82, 85, 88, 90, 92, 94, 96],
                    line: {color: '#B85170', width: 3, shape: 'spline'}, // Changed to primary color
                    marker: {symbol: 'heart', size: 10}
                  },
                  {
                    type: 'scatter',
              ...
              <Plot
                data={[
                  ...
                  },
                  {
                    ...
                    x: ['1st Tri', '2nd Tri', '3rd Tri'],
                    y: [82, 88, 94],
                    line: {color: '#A04763', width: 3, shape: 'spline'}, // Darker shade of primary color
                    marker: {symbol: 'star', size: 12, color: '#A04763'}, // Darker shade of primary color
                    yaxis: 'y2'
                  }
                ]}
                layout={{
                  ...
                  plot_bgcolor: '#FFF5F5', // Adjusted to light pink background
                  paper_bgcolor: 'rgba(0,0,0,0)',
                  xaxis: { title: 'Trimester', gridcolor: 'rgba(184,81,112,0.1)' }, // Adjusted grid color
                  yaxis: { title: 'BP (mmHg)', gridcolor: 'rgba(184,81,112,0.1)' },
                  yaxis2: { title: 'Glucose (mg/dL)', overlaying: 'y', side: 'right', titlefont: {color: '#A04763'} } // Darker shade of primary color
                }}
                config={{displayModeBar: false}}
              />
            </div>
...
const styles = {
  ...
  content: {
    ...
    width: '100%',
    maxWidth: '1400px',
    backgroundColor: '#FFFFFF', // White background like Newsletter content
    boxShadow: '0 4px 10px rgba(0, 0, 0, 0.1)', // Added shadow from Newsletter
  },
  mainHeading: {
    fontSize: '38px',
    color: '#B85170', // Changed to primary color
