          ...
          <ul style={styles.notificationList}>
            {notifications.map((notification) => (
              <li key={notification._id} style={{
                ...styles.notificationItem,
                backgroundColor: notification.isRead ? '#f0f0f0' : '#f9f9f9',
                borderLeft: notification.isRead ? '1px solid #ddd' : '3px solid #B15870' // Changed from #ff8c00
              }}>
                <strong>{notification.type === 'medication_reminder' ? 'Medication Reminder' : 'Appointment Reminder'}:</strong> {notification.message}
                <br />
                <small>{new Date(notification.createdAt).toLocaleString()}</small>
...
const styles = {
  container: {
    ...
    fontFamily: "'Poppins', sans-serif",
    boxSizing: 'border-box',
    paddingTop: '90px', // Adjusted to match other components with navbar
  ...
  overlay: {
    ...
    right: 0,
    bottom: 0,
    backgroundColor: 'rgba(253, 232, 233, 0.7)', // Same overlay as others
    zIndex: -1,
  },
  notificationsContainer: {
    backgroundColor: '#FFFFFF', // Changed to pure white to match others
    padding: '40px',
    borderRadius: '10px',
    boxShadow: '0 4px 10px rgba(0, 0, 0, 0.1)',
    textAlign: 'center',
  ...
  info: {
    fontSize: '16px',
    color: '#000000', // Changed to black to match other components
  },
  error: {
    fontSize: '16px',
    color: '#FF4444', // Darker red for visibility, consistent with Profile
    marginBottom: '20px',
  },
  success: {
    fontSize: '16px',
    color: '#4CAF50', // Kept green as standard for success
    marginBottom: '20px',
  },
  notificationList: {
    listStyleType: 'none',
